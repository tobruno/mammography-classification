...
for index, row in df.iterrows():
    ...
    # Read and preprocess the attention mask
...
f = open("Data/test_images_resnet.txt", "w")
f.write("\n".join(testPaths))
f.close()
...
# ROI input branch
roi_input = Input(shape=(224, 224, 1))
attention_branch = Conv2D(filters=256, kernel_size=(3, 3), strides=(2, 2), activation='relu',
...
attention_branch = Resizing(height=56, width=56)(attention_branch)
resnet_layer_to_combine = base_model.get_layer('conv2_block2_out').output # Example layer index
...
model = Model(inputs=(base_model.input, roi_input), outputs=(class_output, circle_output))
...
# Train the model
H = model.fit(
	[trainImages, trainMasks], trainTargets,
	validation_data=([testImages, testMasks], testTargets),
    batch_size=32,
    epochs=15,
    verbose=1,
